...
module.exports = {
	...
	infoLogger:async(info, url, datetime)=>{
		dashLogger.info(`Info : ${info}, Request : ${url}`, { datetime: momFormat(datetime) })
	},
	errorLogger:async(error,url,datetime)=>{
	   dashLogger.error(`Error : ${error}, Request : ${url}`, { datetime: momFormat(datetime) })
	},
	fileUpload:async(tmp_path,target_path)=>{
